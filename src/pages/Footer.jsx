...
function Footer() {
  ...
  return (
    <footer className="py-10 px-4 sm:px-6 lg:px-8 font-inter relative overflow-hidden bg-gradient-to-r from-gray-900 via-black to-gray-900 border-t border-gray-700/50">
      <div className="max-w-7xl mx-auto flex flex-col items-center relative z-10">
        ...
        <div className="mb-6 flex items-center justify-center">
          ...
          {/* Updated Logo Text to match portfolio theme */}
          <span className="text-white text-3xl font-extrabold tracking-wide">Portfolio</span>
        </div>
          ...
          <ul className="flex flex-wrap justify-center gap-x-4 gap-y-1 text-base font-medium">
            {navLinks.map((link) => (
              <li key={link.name}>
                <a
                  href={link.href}
                  className="text-gray-400 hover:text-white transition-all duration-300 relative after:absolute after:left-0 after:bottom-0 after:h-0.5 after:w-0 after:bg-blue-400 after:transition-all after:duration-300 hover:after:w-full"
                >
                  {link.name}
                </a>
              </li>
        ...
        <p className="text-center text-xs text-gray-500 mt-4">
          &copy; {new Date().getFullYear()} Khem Sopheanan Portfolio. All rights reserved.
        </p>
      </div>
    </footer>
  );
